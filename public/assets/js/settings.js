...
export async function renderSettings(container) {
    ...
    const pwForm = container.querySelector('[data-role="pw-form"]');
    ...
    container.querySelector('[data-role="import"]')?.addEventListener('click', () => openImport(container));
...
async function save(container, form) {
    ...
    if (v.lmstudio_api_key === SECRET_UNCHANGED)
        delete v.lmstudio_api_key;
    try {
 ...
 */
function wireAppearance(form) {
    const section = form.querySelector('[data-role="appearance"]');
    if (!section)
        return;
    ...
    section.querySelector('[data-role="reduce"]')?.addEventListener('change', (e) => {
        field('ui_transparency').value = e.target.checked ? 'reduced' : 'full';
        preview();
    });
    let timer = 0;
    field('ui_wallpaper_url').addEventListener('input', () => {
        ...
        timer = window.setTimeout(() => {
            ...
            const thumb = section.querySelector('.wp-thumb[data-value="custom"]');
            if (thumb)
                thumb.style.backgroundImage = /^https?:\/\//i.test(url) ? cssUrl(url) : '';
            preview();
        }, 350);
    });
}
...
function openImport(container) {
    ...
    const $ = (role) => backdrop.querySelector(`[data-role="${role}"]`);
    const report = $('report');
    ...
    $('file').addEventListener('change', async (e) => {
        ...
        try {
            ...
            const res = await apiPost('import', 'inspect', { file: text });
        ...
        <div style="margin-top:6px">${total} rows: ${rows.map(([t, n]) => `${escapeHtml(t.replace(/_/g, ' '))} ${n}`).join(' · ')}</div>
        ${res.warnings.length ? `<div class="text-warn" style="margin-top:8px">${res.warnings.map((w) => escapeHtml(w)).join('<br>')}</div>` : ''}`;
    ...
    $('go').addEventListener('click', async () => {
        ...
        try {
            const res = await apiPost('import', 'apply', {
