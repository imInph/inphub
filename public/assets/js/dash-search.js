/**
 * inphub: the dashboard search bar's dropdown, Google suggestions (up to 5)
...
import { escapeHtml, money } from './ui.js?v=e4ef27a132';
 ...
 */
export function mountDashSearch(container, keep) {
    const form = container.querySelector('.dash-search');
    const input = form?.querySelector('[data-role="search"]');
    const list = form?.querySelector('[data-role="suggest"]');
    if (!form || !input || !list)
        return;
    ...
    const paint = () => {
        list.querySelectorAll('[data-i]').forEach((el, i) => el.classList.toggle('active', i === active));
        const row = list.querySelector(`[data-i="${active}"]`);
        if (row) {
    ...
    const render = () => {
        ...
        const html = [];
        if (google.length) {
            html.push('<div class="palette-section">Google</div>');
            for (const s of google.slice(0, GOOGLE_ROWS)) {
                html.push(row(entries.length, `<span class="palette-label"><span>${suggestionHtml(q, s)}</span></span>`));
        ...
        if (results.length) {
            html.push('<div class="palette-section">In inphub</div>');
            for (const r of results) {
                ...
                const meta = r.item.amount !== undefined
                    ...
                    : r.item.meta;
                html.push(row(entries.length, `<span class="palette-label">${escapeHtml(r.item.label)}<small>${escapeHtml(sub)}</small></span>`
