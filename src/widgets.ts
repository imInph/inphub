...
export interface DashData {
  ...
  upcoming: {
    todos: { id: number; title: string; priority: string; due_date: string }[];
  ...
  form?.addEventListener('submit', async (e) => {
    ...
    try {
      if (captureMode === 'todo') {
        ...
        const res = await apiPost<{ id: number }>('notes', 'create', { content: text });
...
function renderUpcoming(d: DashData): string {
  ...
  type Entry = { day: string; html: string };
  const entries: Entry[] = [
    ...up.todos.map((t) => ({
      day: t.due_date,
...
export function abortBrief(): void {
  briefCtl?.abort();
  briefCtl = null;
}

function briefHost(container: HTMLElement): HTMLElement | null {
  return container.querySelector<HTMLElement>('[data-role="brief"]');
}

function briefBody(content: string | null): string {
...
export async function clearBrief(container: HTMLElement): Promise<void> {
  const host = briefHost(container);
  if (!host) return;
  try {
...
export async function generateBrief(container: HTMLElement): Promise<void> {
  const host = briefHost(container);
  if (!host || briefCtl) return; // one request at a time
  ...
  try {
    const res = await apiPost<{ brief: { content: string } }>('ai', 'generate_brief', {}, { signal: briefCtl.signal });
...
export const WIDGETS = {
  ...
  brief: {
    ...
    mount: (container: HTMLElement) => { void loadBrief(container); },
  },
  activity: {
...
export function widgetAvailable(id: WidgetId): boolean {
  return !(WIDGETS[id] as WidgetDef).aiOnly || aiAvailable;
}
