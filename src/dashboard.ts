...
import { escapeHtml, toast, onAction, openModal, formValues, loadingState } from './ui.js';
import { go } from './app.js';
import { icon } from './icons.js';
import {
  WIDGETS, widgetShell, widgetAvailable, isWidgetId, setCaptureMode, generateBrief, clearBrief, abortBrief,
...
export async function renderDashboard(container: HTMLElement): Promise<void> {
    ...
    <div class="dash${firstPaint ? ' dash-enter' : ''}">
      ...
      <div class="dash-body">
        <div class="dash-widgets" data-role="widgets">
          ${layout.length
            ? layout.map((l, i) => {
                const def: WidgetDef = WIDGETS[l.id];
  ...
  onAction(container, (action, el, ev) => {
    ...
    if (action === 'customize') openWidgetPicker(data?.layout ?? [], shortcuts, () => void rerender());
