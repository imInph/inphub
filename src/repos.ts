...
export async function renderRepos(container: HTMLElement): Promise<void> {
  ...
  onAction(container, (action, el) => {
    ...
    if (action === 'analyze-stale') analyzeStale(container);
  });
...
async function openDetail(id: number): Promise<void> {
  const res = await apiGet<{ repo: Repo; suggestions: Suggestion[] }>('repos', 'detail', { id });
  const r = res.repo;
...
async function analyzeStale(container: HTMLElement): Promise<void> {
  const btn = container.querySelector<HTMLButtonElement>('[data-action="analyze-stale"]');
  if (btn) {
    btn.disabled = true;
    btn.textContent = 'Analyzing…';
  }
