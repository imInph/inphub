...
import { escapeHtml, money, fmtMonth, fmtDate, emptyState, loadingState, onAction } from './ui.js';

declare const Chart: any;

interface Weekday { dow: number; total: number; count: number; }
interface HabitRate { id: number; name: string; color: string; done_days: number; rate: number; }
interface Point { d: string; value: number; }
interface HourBucket { hour: number; count: number; }
interface CategoryTotal { name: string; color: string | null; total: string; }

interface Insights {
  period: string; label: string; from: string | null; to: string | null;
  days: number; currency: string; granularity: 'day' | 'month';
  totals: {
...
async function load(container: HTMLElement): Promise<void> {
  ...
  const d = await apiGet<Insights>('insights', 'summary', { period });
...
function drawCharts(container: HTMLElement, d: Insights, cur: string): void {
  ...
  make('hours', {
    ...
    options: {
      plugins: {
        ...
        tooltip: {
          callbacks: {
            title: (items: any) => `${String(d.hours[items[0].dataIndex].hour).padStart(2, '0')}:00`,
  ...
  const label = (p: Point) => (d.granularity === 'month' ? fmtMonth(p.d) : fmtDate(p.d));
