...
import { escapeHtml, timeAgo, fmtDate, emptyState, onAction, loadingState } from './ui.js';
import { go } from './app.js';

interface Activity {
  id: number; type: string; entity_type: string | null; entity_id: number | null;
  summary: string; actor: string; metadata: string | null; created_at: string;
}
...
async function load(container: HTMLElement): Promise<void> {
  const items = await apiGet<Activity[]>('activity', 'list', {
    limit,
    ...(actor ? { actor } : {}),
